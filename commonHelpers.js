var x=(t,o,e)=>{if(!o.has(t))throw TypeError("Cannot "+e)};var l=(t,o,e)=>(x(t,o,"read from private field"),e?e.call(t):o.get(t)),p=(t,o,e)=>{if(o.has(t))throw TypeError("Cannot add the same private member more than once");o instanceof WeakSet?o.add(t):o.set(t,e)};import{i as T,a as _,b as M,c as N,d as O}from"./assets/dark-light-switcher-4a551f72.js";import{a as u,N as w,n as q}from"./assets/vendor-83fc1255.js";function R(t){const o=document.getElementById("bookModal");o.innerHTML="";const e=document.createElement("div");e.classList.add("modal-content"),e.innerHTML=`
    ...
    </li>`}function J(t){const o=[];for(let e=0;e<t.length;e+=1){const s=t[e].books.map((n,r)=>{if(r<5)return C(n)}).join(""),i=`<h2 class="best-category-title">${t[e].list_name}</h2>
    <ul class="best-category-list">${s}</ul>
    <div class="see-more-btn-box">
    <button class="see-more-btn" name="${t[e].list_name}" type="button">See more</button></div>`;o.push(i)}H.innerHTML=o.join("")}const U=new h;async function $(){k();try{const t=await U.getBooks();t.length===0&&w.Notify.warning("Sorry, there are no bestsellers books yet"),J(t),document.querySelectorAll(".book-card-box").forEach(e=>{e.addEventListener("click",()=>{const s=JSON.parse(e.dataset.bookInfo);R(s)})})}catch{w.Notify.failure("Something went wrong!")}finally{k()}}$();const E=(t,o)=>{const e=document.createElement("li");e.classList.add("categories-item");const s=`categoryRadio${o+1}`,i=document.createElement("input");i.setAttribute("type","radio"),i.setAttribute("name","category"),i.setAttribute("id",s),i.classList.add("categories-button");const n=document.createElement("label");return n.textContent=t.list_name,n.setAttribute("for",s),n.classList.add("categories-label"),e.appendChild(i),e.appendChild(n),e};async function z(){const t=await new h().getAllCategories();t.sort((e,s)=>e.list_name.localeCompare(s.list_name));const o=document.querySelector(".categories-list");t.forEach((e,s)=>{const i=E(e,s);o.appendChild(i)});try{const e=await new h().getAllCategories();e.sort((i,n)=>i.list_name.localeCompare(n.list_name));const s=document.querySelector(".categories-list");e.forEach((i,n)=>{const r=E(i,n);s.appendChild(r)})}catch{q.Notify.failure("Something went wrong!")}}z();const F=new h,y=document.querySelector(".book-block-list"),v=document.querySelector(".book-block-title"),S=document.querySelector(".categories-list");y==null||y.addEventListener("click",j);function j(t){t.preventDefault(),window.innerWidth>=1440?window.scrollTo(window.innerHeight,0):window.innerWidth>=768&&window.innerWidth<1440?window.scrollTo(window.innerHeight,500):window.scrollTo(window.innerHeight,820),document.querySelectorAll("label").forEach(e=>{e.textContent===t.target.name?e.classList.add("active"):e.classList.remove("active")}),t.target.nodeName==="BUTTON"&&(v.style.display="none",A(t.target.name))}S==null||S.addEventListener("click",D);function D(t){t.preventDefault(),document.querySelectorAll("label").forEach(s=>{s===t.target&&s.classList.add("active"),s.classList.remove("active")});const e=t.target;e.nodeName==="LABEL"&&e.textContent!=="All Categories"?(e.classList.add("active"),v.style.display="none",A(e.textContent)):e.textContent==="All Categories"&&(v.style.display="block",e.classList.add("active"),$())}async function A(t){try{k();const o=await F.getBooksFromCategory(t);G(o,t)}catch{w.Notify.failure("Sorry, no books in this category")}finally{k()}}function G(t,o){const e=t.map(g=>C(g)).join(""),s=o.split(" ");let i=s.length-1,n=s[i];const m=`<h2 class="book-block-title">${s.splice(0,i).join(" ")}&nbsp;<span class="book-block-title-accent">${n}</span>
    </h2>
