var I=(t,o,e)=>{if(!o.has(t))throw TypeError("Cannot "+e)};var l=(t,o,e)=>(I(t,o,"read from private field"),e?e.call(t):o.get(t)),g=(t,o,e)=>{if(o.has(t))throw TypeError("Cannot add the same private member more than once");o instanceof WeakSet?o.add(t):o.set(t,e)};import{i as x,a as T,b as _,c as M,d as N}from"./assets/dark-light-switcher-4a551f72.js";import{a as u,N as S,n as O}from"./assets/vendor-83fc1255.js";function q(t){const o=document.getElementById("bookModal");o.innerHTML="";const e=document.createElement("div");e.classList.add("modal-content"),e.innerHTML=`
    ...
    <button class="see-more-btn" name="${t[e].list_name}" type="button">See more</button></div>`;o.push(i)}R.innerHTML=o.join("")}const J=new $;async function E(){k();try{const t=await J.getBooks();t.length===0&&S.Notify.warning("Sorry, there are no bestsellers books yet"),H(t),document.querySelectorAll(".book-card-box").forEach(e=>{e.addEventListener("click",()=>{const s=JSON.parse(e.dataset.bookInfo);q(s)})})}catch{S.Notify.failure("Something went wrong!")}finally{k()}}E();const U=(t,o)=>{const e=document.createElement("li");e.classList.add("categories-item");const s=`categoryRadio${o+1}`,i=document.createElement("input");i.setAttribute("type","radio"),i.setAttribute("name","category"),i.setAttribute("id",s),i.classList.add("categories-button");const n=document.createElement("label");return n.textContent=t.list_name,n.setAttribute("for",s),n.classList.add("categories-label"),e.appendChild(i),e.appendChild(n),e};async function z(){try{const t=await new $().getAllCategories();t.sort((e,s)=>e.list_name.localeCompare(s.list_name));const o=document.querySelector(".categories-list");t.forEach((e,s)=>{const i=U(e,s);o.appendChild(i)})}catch{O.Notify.failure("Something went wrong!")}}z();const F=new $,y=document.querySelector(".book-block-list"),w=document.querySelector(".book-block-title"),B=document.querySelector(".categories-list");y==null||y.addEventListener("click",j);function j(t){t.preventDefault(),window.innerWidth>=1440?window.scrollTo(window.innerHeight,0):window.innerWidth>=768&&window.innerWidth<1440?window.scrollTo(window.innerHeight,500):window.scrollTo(window.innerHeight,820),document.querySelectorAll("label").forEach(e=>{e.textContent===t.target.name?e.classList.add("active"):e.classList.remove("active")}),t.target.nodeName==="BUTTON"&&(w.style.display="none",C(t.target.name))}B==null||B.addEventListener("click",D);function D(t){t.preventDefault(),document.querySelectorAll("label").forEach(s=>{s===t.target&&s.classList.add("active"),s.classList.remove("active")});const e=t.target;e.nodeName==="LABEL"&&e.textContent!=="All Categories"?(e.classList.add("active"),w.style.display="none",C(e.textContent)):e.textContent==="All Categories"&&(w.style.display="block",e.classList.add("active"),E())}async function C(t){try{k();const o=await F.getBooksFromCategory(t);G(o,t)}catch{S.Notify.failure("Sorry, no books in this category")}finally{k()}}function G(t,o){const e=t.map(d=>v(d)).join(""),s=o.split(" ");let i=s.length-1,n=s[i];const m=`<h2 class="book-block-title">${s.splice(0,i).join(" ")}&nbsp;<span class="book-block-title-accent">${n}</span>
    </h2>
